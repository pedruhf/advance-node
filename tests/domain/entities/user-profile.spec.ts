import { UserProfile } from "@/domain/entities";

describe("UserProfile", () => {
  let sut: UserProfile;

  beforeEach(() => {
    sut = new UserProfile("any_id");
  });

  test("Should create without initials when pictureUrl is provided", () => {
    sut.setPicture({ pictureUrl: "any_url", name: "any_name" });

    expect(sut).toEqual({
      id: "any_id",
      pictureUrl: "any_url",
      initials: undefined,
    });
  });

  test("Should create without initials when pictureUrl is provided", () => {
    sut.setPicture({ pictureUrl: "any_url" });

    expect(sut).toEqual({
      id: "any_id",
      pictureUrl: "any_url",
      initials: undefined,
    });
  });

  test("Should create initials with first letter of first and last names", () => {
    sut.setPicture({ name: "Pedro Henrique de Freitas Silva" });

    expect(sut).toEqual({
      id: "any_id",
      pictureUrl: undefined,
      initials: "PS",
    });
  });

  test("Should create initials with first two letters of first name", () => {
    sut.setPicture({ name: "pedro" });

    expect(sut).toEqual({
      id: "any_id",
      pictureUrl: undefined,
      initials: "PE",
    });
  });

  test("Should create initials with first letter", () => {
    sut.setPicture({ name: "p" });

    expect(sut).toEqual({
      id: "any_id",
      pictureUrl: undefined,
      initials: "P",
    });
  });

  test("Should with empty initials when name and pictureUrl are not provided", () => {
    sut.setPicture({ name: "" });

    expect(sut).toEqual({
      id: "any_id",
      pictureUrl: undefined,
      initials: undefined,
    });
  });

  test("Should with empty initials when name and pictureUrl are not provided", () => {
    sut.setPicture({});

    expect(sut).toEqual({
      id: "any_id",
      pictureUrl: undefined,
      initials: undefined,
    });
  });
});
